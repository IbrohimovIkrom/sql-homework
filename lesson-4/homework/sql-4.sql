...
select * from employees where age between 30 and 40 and 'Department' = 'Marketing'

--Use OFFSET-FETCH to select rows 11 to 20 from the Employees table, ordered by Salary DESC.  

select * from employees order by EmployeeID offset 11 rows fetch next 20 rows only
...
--Write a query that filters the Orders table for orders placed in the last 180 days using BETWEEN and LATEST_DATE in the table.
